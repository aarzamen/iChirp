...
| [011](2026-09-22-011-m2-dictation.md) | M2: dictation | **IN PROGRESS** on `m2/dictation` — Steps 1–7 and Step 8's `UIBackgroundModes: audio` built (focused package tests, 25 simulator app tests, lint clean); dictation works end to end in the Simulator (live preview, final pass, clipboard, Live Activity Stop); widget extension with a plain bundle id, no entitlement | Device checks (M2 QA checklist, Step 8 research note); full package suite and `device_smoke.sh` at merge; migration `v4-dictation-text` must register after M4's `v3-language-models` |
| [012](2026-09-22-012-m3-meetings.md) | M3: meeting recording | **IN PROGRESS** on `m3/meetings` — Steps 1–7 and the Live Activity built (focused package tests, 25 simulator app tests, lint clean); simulator: record, pause, notes, kill -9 → Recover with Partial audio, Lock Screen Pause / Stop & save; migration `v5-meetings` | Device checks (M3 QA checklist: 60-minute locked meeting, kill → Recover, calls, AirPods); full package suite and `device_smoke.sh` at merge; `v5-meetings` must register before M5's `v6-documents` |
| [013](2026-09-22-013-m4-language-models-and-deliverables.md) | M4: language models and deliverables | **IN PROGRESS** — Steps 1–5 (engines, Keychain keys, provider settings, routing with per-run overrides, templates, deliverables, ledger, map-reduce) merged; Step 6 on `m4/language-models-ui`: screens (Transcript privacy chip, Ask, Transform, Transforms tab, Settings → Models), Info.plist local-network keys, `project.yml` products, app-hosted Keychain and clinical-confirmation tests, simulator screen tour; ADR-011 | Owner device QA (human QA guide, M4 checklist): Apple Intelligence on the 17 Pro, a LAN Ollama / LM Studio, one cloud provider |
| [014](2026-09-22-014-m5-ingest.md) | M5: ingest breadth | **IN PROGRESS** on `m5/ingest` — Steps 1–6 built: link classifier, resumable downloads, Apple Podcasts (lookup + RSS fallback), YouTube captions, PDF with Vision OCR, TXT/MD/RTF/HTML/DOCX (no new dependency), Paste a link sheet, document rows and screen, share-sheet document types; migration `v6-documents`; focused package tests (385 in touched modules) and 28 simulator app tests green, lint clean | **Owner decision: YouTube audio** (options in plan Step 3); device checks (podcast, MP3, YouTube, scanned PDF, DOCX); full package suite and `scripts/test.sh` at merge; `v6-documents` registers after M3's `v5-meetings` |
