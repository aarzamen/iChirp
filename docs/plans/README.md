...
| [016](2026-09-22-016-m7-engine-breadth.md) | M7: engine breadth and benchmarks | **IN PROGRESS**. ASR lane `lane/asr-engines`: Step 1 is the registry, live and final routes, meeting lease and Settings → Speech engines; Step 2 is Apple SpeechTranscriber; Step 4 is WhisperKit on `argmax-oss-swift` exact 1.1.0 (Base, Large v3 Turbo); Step 6 is the benchmark harness and screen, with Mac numbers in [asr-engine-benchmarks](../research/2026-09-22-asr-engine-benchmarks.md). Focused suites green, lint clean. Step 3 (streaming) is not built; Step 5 (small language models) is another lane | Controller: iPhone checks (Apple Speech, Turbo memory, benchmark numbers), full suite at merge, device smoke |
