...
- **Status:** IN PROGRESS, merged on `ichirp/foundation`. The ASR lane (`lane/asr-engines`) built Steps 1, 2, 4 and
  6. Step 5 (small language models) was built on `lane/on-device-llm`: llama.cpp from pinned source
  ([ADR-015](../../spec/adr/015-on-device-llm-llama-cpp.md)), Qwen3.5 2B default and Qwen3 4B Instruct 2507 quality
  tier (Apache-2.0; LFM2.5 dropped on license), MLX Swift spiked and not adopted; Mac numbers in
