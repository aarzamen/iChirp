...
- An override is a deliberate, per-run confirmation ("Send this clinical note to <provider>?"), never a remembered
  setting. The app logs that an override happened (provider, time, item id), **never the content**.
...
| Cloud language models (Anthropic, OpenAI-compatible, Gemini) | User runs a template or Ask with a cloud provider | Transcript or document **text** and the prompt; **never audio** | M4 |
...
- API keys live in the **Keychain** (never `UserDefaults`, never files, never logs). The Gemini port's plaintext
  `@AppStorage` key is a rejected pattern.
