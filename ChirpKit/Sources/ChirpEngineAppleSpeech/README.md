...
- `AppleSpeechEngine.swift`: the `SpeechEngine` and `SpeechEngineAvailabilityReporting` actor.
  ...
  - **Transcribe:** `prepare` and `transcribe` never download. They throw `modelNotDownloaded` while the model is
    missing, and ask for Speech Recognition permission only when iOS has never asked.
