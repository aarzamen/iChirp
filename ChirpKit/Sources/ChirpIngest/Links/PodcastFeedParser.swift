...
public enum PodcastFeedParser {
    ...
    public static func parse(_ data: Data) throws -> [PodcastFeedEpisode] {
        try parseFeed(data).episodes
    }

    /// The channel's title (nil when absent) and its playable episodes, in feed order.
    public static func parseFeed(_ data: Data) throws -> (title: String?, episodes: [PodcastFeedEpisode]) {
        ...
        return (firstNonEmpty(delegate.channelTitle), delegate.episodes)
    }
    ...
    private final class FeedDelegate: NSObject, XMLParserDelegate {
        ...
        /// The channel's `<title>` (text of a `title` element outside every `<item>`, before the first item).
        ...
        func parser(
            ...
            if name == "title", !inItem, !sawItem, channelTitle.isEmpty {
                inChannelTitle = true
                return
            }
            if name == "item" {
