// Ported from MacParakeet (GPL-3.0): Sources/MacParakeetCore/Services/Dictation/LiveTranscriptStabilizer.swift @ bbae9e0e
// Changes: public (ChirpText owns it; the dictation coordinator in ChirpFeatures drives it), and `committedText` /
// `tentativeText` so the Dictating screen can dim the tentative tail. The alignment logic is unchanged.
...
///
/// Not thread-safe by itself — its owner (the dictation coordinator, on the main
/// actor) makes every call under one isolation.
public struct LiveTranscriptStabilizer: Sendable {
    /// Words committed so far, in order. Append-only within a session.
    public private(set) var committedWords: [String] = []
    ...
    public init(hypothesisHoldback: Int = 3, anchorLength: Int = 6, maxCommittedWords: Int = 120) {
    ...
    @discardableResult
    public mutating func ingest(_ raw: String) -> String {
    ...
    /// The settled words, shown at full strength.
    public var committedText: String {
        committedWords.joined(separator: " ")
    }

    /// The last few words, still forming; shown dimmed after `committedText`.
    public var tentativeText: String {
        hypothesisWords.joined(separator: " ")
    }
