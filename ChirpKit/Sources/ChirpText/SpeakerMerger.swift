// Ported from MacParakeet (GPL-3.0): Sources/MacParakeetCore/Services/Diarization/SpeakerMerger.swift @ bbae9e0e
// Changes: `SpeakerSegment` replaced by ChirpCore's `DiarizationSegmentRecord` (same shape); no other
// adaptations.
...
public enum SpeakerMerger {
    ...
    public static func mergeWordTimestampsWithSpeakers(
        ...
        // Defensive sort — the diarizer returns chronological output, but the
        // algorithm requires sorted input for correctness.
    ...
    ///
    /// Over-split clustering produces singleton IDs that `TranscriptSegmenter` then
    /// renders as their own bubble. A word between two different speakers is left
    /// alone: that is the two-talker short, not a bubble.
