// Ported from MacParakeet (GPL-3.0): Tests/MacParakeetTests/Utilities/TranscriptSegmenterTests.swift @ bbae9e0e
// Changes: `WordTimestamp`/`SpeakerInfo`/`DiarizationSegmentRecord`/`TranscriptSegmentWordRange` come
// from ChirpCore. Skipped `testMaterializeSegmentsUsesSourceLabelsWhenSpeakerRosterIsAbsent` — it
// pinned the upstream `AudioSource` speaker-label fallback ("microphone" → "Me", "system" → "Others"),
// which was not ported; ChirpText's fallback is the speakerId itself (see ChirpText/README.md).
