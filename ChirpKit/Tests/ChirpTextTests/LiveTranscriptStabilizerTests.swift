// Ported from MacParakeet (GPL-3.0): Tests/MacParakeetTests/LiveTranscriptStabilizerTests.swift @ bbae9e0e
// Changes: imports ChirpText; one added test for the committed/tentative split the Dictating screen dims.
...
final class LiveTranscriptStabilizerTests: XCTestCase {
    ...
    // MARK: - iChirp: committed and tentative parts

    func testCommittedAndTentativeTextSplitAtTheHoldback() {
        var s = makeStabilizer()
        _ = s.ingest("the quick brown fox jumps")
        XCTAssertEqual(s.committedText, "the quick")
        XCTAssertEqual(s.tentativeText, "brown fox jumps")
        s.reset()
        XCTAssertEqual(s.committedText, "")
        XCTAssertEqual(s.tentativeText, "")
    }
}
