...
struct StubResponse: Sendable {
    ...
    static func body(_ text: String, status: Int = 200, contentType: String = "text/event-stream") -> StubResponse {
        StubResponse(status: status, headers: ["Content-Type": contentType], chunks: [Data(text.utf8)])
    }

    static func lines(_ lines: [String], contentType: String = "text/event-stream") -> StubResponse {
        StubResponse(headers: ["Content-Type": contentType], chunks: lines.map { Data(($0 + "\n").utf8) })
    }
}
